#version 330 core

struct Material
{
    sampler2D diffuse;
    sampler2D specular;
    float shininess;
};

struct lightMaterial
{
    vec3 position;
    vec3 direction;
    vec3 ambient;
    vec3 diffuse;
    vec3 specular;
    float innerCutoff;
    float outerCutoff;
    float constant;
    float linear;
    float quadratic;
};

in vec3 fragPos;
in vec3 normal;
in vec2 texCoords;

out vec4 final_color;

uniform Material mat;
uniform lightMaterial lMat;

uniform vec3 camPos;

void main()
{
    //AMBIENT
    vec3 ambient = lMat.ambient * texture(mat.diffuse, texCoords).rgb;

    vec3 lightDir = lMat.position - fragPos;
    float distance = length(lightDir);
    lightDir = normalize(lightDir);
    float theta = dot(lightDir, normalize(-lMat.direction));
    float epsilon = lMat.innerCutoff - lMat.outerCutoff;
    float intensity = theta - lMat.outerCutoff;
    intensity = intensity / epsilon;
    intensity = clamp(intensity, 0.0f, 1.0f);

    //DIFFUSE
    vec3 norm = normalize(normal);
    float diff = dot(norm, lightDir);
    diff = max(diff, 0.0f);
    vec3 diffuse = intensity * lMat.diffuse * diff * texture(mat.diffuse, texCoords).rgb;

    //SPECULAR
    vec3 viewDir = normalize(camPos - fragPos);
    vec3 reflectedLightDir = reflect(-lightDir, norm);
    float spec = dot(reflectedLightDir, viewDir);
    spec = max(spec, 0.0f);
    spec = pow(spec, mat.shininess);
    vec3 specular = intensity * lMat.specular * spec * texture(mat.specular, texCoords).rgb;

    float attenuation = lMat.constant + lMat.linear * distance + lMat.quadratic * distance * distance;
    attenuation = 1.0 / attenuation;

    ambient  *= attenuation; 
    diffuse  *= attenuation;
    specular *= attenuation; 

    final_color = vec4(ambient + diffuse + specular, 1.0f);
}